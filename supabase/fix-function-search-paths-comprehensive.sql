-- 🔧 Fix Function Search Path Mutability Warnings (Comprehensive Version)
-- =====================================================
-- 
-- This script comprehensively fixes all function search path mutability warnings
-- by dynamically finding and handling all trigger dependencies
-- =====================================================

-- First, let's find and drop ALL triggers that depend on functions we need to fix
-- This ensures we don't miss any dependencies

-- Drop triggers for update_lab_work_updated_at function
...
DROP TRIGGER IF EXISTS trigger_update_lab_work_updated_at ON lab_work;
DROP TRIGGER IF EXISTS update_lab_work_updated_at ON lab_work_orders;
DROP TRIGGER IF EXISTS update_lab_work_updated_at ON lab_work_results;
DROP TRIGGER IF EXISTS update_lab_work_updated_at ON lab_work;

-- Drop triggers for update_system_settings_updated_at function
DROP TRIGGER IF EXISTS update_system_settings_updated_at ON system_settings;
DROP TRIGGER IF EXISTS trigger_update_system_settings_updated_at ON system_settings;

-- Drop triggers for auto_link_appointment_with_patient function
DROP TRIGGER IF EXISTS auto_link_appointment_with_patient ON appointments;
DROP TRIGGER IF EXISTS auto_link_appointment_trigger ON appointments;
DROP TRIGGER IF EXISTS trigger_auto_link_appointment ON appointments;

-- Now we can safely drop and recreate functions

-- 1. Fix add_treatment_payment function
DROP FUNCTION IF EXISTS add_treatment_payment(UUID, UUID, DECIMAL, TEXT, TEXT);
...
DROP FUNCTION IF EXISTS create_lab_work_order(UUID, UUID, TEXT, TEXT, DATE);
...
DROP FUNCTION IF EXISTS add_lab_work_result(UUID, JSONB, TEXT, TEXT);
...
DROP FUNCTION IF EXISTS get_lab_work_summary(UUID);
...
DROP FUNCTION IF EXISTS log_system_change(TEXT, TEXT, TEXT, JSONB, JSONB, TEXT, INET, TEXT);
...
DROP FUNCTION IF EXISTS get_lab_work_status_history(UUID);
...
DROP FUNCTION IF EXISTS is_same_person(TEXT, TEXT);
...
DROP FUNCTION IF EXISTS find_or_create_patient_improved(TEXT, TEXT, TEXT, DATE, TEXT, TEXT);
...
-- 11. Fix update_lab_work_updated_at function (now safe to drop)
DROP FUNCTION IF EXISTS update_lab_work_updated_at();
...
DROP FUNCTION IF EXISTS create_prescription_with_history(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
...
DROP FUNCTION IF EXISTS refill_prescription(UUID);
...
DROP FUNCTION IF EXISTS update_lab_work_status(UUID, TEXT, TEXT);
...
-- 15. Fix auto_link_appointment_with_patient function (now safe to drop)
...
DROP FUNCTION IF EXISTS get_treatment_payments(UUID);
...
DROP FUNCTION IF EXISTS get_feature_toggles();
...
DROP FUNCTION IF EXISTS get_lab_work_orders(UUID);
...
DROP FUNCTION IF EXISTS get_lab_work_results(UUID);
...
-- 21. Fix update_system_settings_updated_at function (now safe to drop)
...
DROP FUNCTION IF EXISTS update_feature_toggle(TEXT, BOOLEAN, TEXT);
...
DROP FUNCTION IF EXISTS get_active_prescriptions(UUID);
...
DROP FUNCTION IF EXISTS get_patient_by_phone(TEXT);
...
DROP FUNCTION IF EXISTS find_or_create_patient(TEXT, TEXT, TEXT);
...
SELECT 'Function Search Path Mutability Fix Complete (Comprehensive Version)!' as status;
