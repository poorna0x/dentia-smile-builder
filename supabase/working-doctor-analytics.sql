...
BEGIN
  ...
  recent_treatments_ranked AS (
    SELECT 
      ...
      dt.cost,
      da.attribution_type,
      ROW_NUMBER() OVER (PARTITION BY da.doctor_id ORDER BY dt.treatment_date DESC) as rn
    FROM doctor_attributions da
    JOIN dental_treatments dt ON da.treatment_id = dt.id
    WHERE dt.clinic_id = clinic_uuid
      AND dt.treatment_date BETWEEN start_date::DATE AND end_date::DATE
  ...
  recent_treatments AS (
    SELECT 
      ...
      ) as recent_treatments
    FROM recent_treatments_ranked
    WHERE rn <= 10
    GROUP BY doc_id
  ),
  ...
  SELECT 
    ...
    COALESCE(
      (SELECT JSONB_OBJECT_AGG(tb.treatment_type, JSONB_BUILD_OBJECT('count', tb.count, 'revenue', tb.revenue, 'percentage', tb.percentage))
       FROM treatment_breakdown tb WHERE tb.doc_id = ds.doctor_id),
      '{}'::JSONB
  ...
  LEFT JOIN attribution_details ad ON ds.doctor_id = ad.doc_id
...
-- Test the function
SELECT 'Testing working doctor analytics function...' as test_step;
...
BEGIN
  ...
  IF clinic_uuid IS NOT NULL THEN
    ...
    RAISE NOTICE '✅ Working doctor analytics function tested successfully for clinic: %', clinic_uuid;
  ELSE
...
-- Success message
SELECT '🎉 Working doctor analytics function created successfully!' as status;
