...
-- Only super admin can read/write system settings
CREATE POLICY "Super admin can manage system settings" ON system_settings
  FOR ALL USING (true); -- This will be controlled by application-level auth
...
CREATE POLICY "Super admin can read audit logs" ON system_audit_log
  FOR SELECT USING (true); -- This will be controlled by application-level auth
...
-- Create a view for easy feature toggle access
...
INSERT INTO system_audit_log (action, entity_type, entity_id, old_values, new_values, user_id) VALUES
('INITIALIZE', 'system_settings', 'feature_toggle', NULL, '{"websiteEnabled": true}', 'system'),
('INITIALIZE', 'system_settings', 'emergency_controls', NULL, '{"emergencyMode": false}', 'system'),
('INITIALIZE', 'system_settings', 'system_monitoring', NULL, '{"enableMonitoring": true}', 'system');

-- Display setup completion message
SELECT 'Super Admin System Setup Complete!' as status;
