-- =====================================================
-- 🦷 PAYMENT SYSTEM FOR DENTAL TREATMENTS
-- =====================================================
...
-- Create function to get lab work payment summary
CREATE OR REPLACE FUNCTION get_lab_work_payment_summary(lab_work_uuid UUID)
RETURNS TABLE (
  total_amount DECIMAL(10,2),
  paid_amount DECIMAL(10,2),
  remaining_amount DECIMAL(10,2),
...
BEGIN
  RETURN QUERY
  SELECT 
    lw.cost as total_amount,
    COALESCE(SUM(pt.amount), 0) as paid_amount,
    lw.cost - COALESCE(SUM(pt.amount), 0) as remaining_amount,
    CASE 
      WHEN COALESCE(SUM(pt.amount), 0) >= lw.cost THEN 'Completed'::payment_status
      WHEN COALESCE(SUM(pt.amount), 0) > 0 THEN 'Partial'::payment_status
      ELSE 'Pending'::payment_status
    END as payment_status
  FROM lab_work lw
  LEFT JOIN payment_transactions pt ON pt.reference_number = lw.id::TEXT
  WHERE lw.id = lab_work_uuid
  GROUP BY lw.id, lw.cost;
END;
$$ LANGUAGE plpgsql;
