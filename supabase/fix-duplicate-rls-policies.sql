-- 🔧 Fix Multiple Permissive RLS Policies (Performance Optimization)
-- =====================================================
-- 
-- This script removes duplicate RLS policies that are causing performance warnings
-- by keeping only the most specific/appropriate policy for each table
-- =====================================================
...
-- Fix lab_work table policies
-- Remove the generic "Allow all operations" policies and keep the specific clinic policies

-- Drop generic policies for lab_work
DROP POLICY IF EXISTS "Allow all operations on lab_work" ON lab_work;

-- Keep the specific clinic policies:
...
SELECT 'Duplicate RLS Policies Cleanup Complete!' as status;
